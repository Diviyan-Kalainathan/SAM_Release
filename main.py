...
from code.metrics.SHD import SHD
...
def score_function(prediction, target):
    ...
    #np.savetxt("results_test.csv", prediction)
...
if __name__ == "__main__":
    ...
    parser.add_argument('--dag', help="The sought graph is a dag", action='store_true')
    ...
    if args.adjmatrix:
        if args.header:
            target = pd.read_csv(args.target, sep=sep).values
        else:
